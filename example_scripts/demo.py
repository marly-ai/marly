...
def read_and_encode_pdfs_folder(folder_path: str) -> Dict[str, str]:
    encoded_pdfs = {}
    ...
    for filename in os.listdir(folder_path):
        if filename.lower().endswith('.pdf'):
            ...
            try:
                ...
                # Generate a unique name based on the file content
                file_hash = hashlib.md5(pdf_content).hexdigest()
                unique_name = f"{os.path.splitext(filename)[0]}_{file_hash[:8]}"

                # Encode the content
                encoded_content = base64.b64encode(zlib.compress(pdf_content)).decode('utf-8')

                unique_name = os.path.splitext(filename)[0]  # Remove the file extension
                encoded_pdfs[unique_name] = encoded_content
                logging.info(f"Successfully processed: {filename} -> {unique_name}")
            except Exception as e:
                logging.error(f"Error processing {filename}: {str(e)}")

    logging.info(f"Total PDFs processed: {len(encoded_pdfs)}")
    return encoded_pdfs
...
def process_pdf_directory(encoded_pdfs):
    
    attention_content = encoded_pdfs['attention_paper']
    lacers_content = encoded_pdfs['lacers']
...
# SINLGE FILES
def process_pdf(file_path, file_path2):
    pdf_content = read_and_encode_pdf(file_path)
    pdf_content2 = read_and_encode_pdf(file_path2)
...
if __name__ == "__main__":
    # Encoded Documents
    # folder_path = "../example_files"
    # encoded_documents = read_and_encode_pdfs_folder(folder_path)
    file_path = "../example_files/CRM-Q2-FY25-Earnings-Presentation.pdf"
